...
function custom_method_manager(){
    ...
    if(btn){
        ...
        let name=modal.querySelector("#nom"),adress=modal.querySelector("#adresse"),detail=modal.querySelector("#detail");
        ...
        addEvent(btn,'click',()=>{
            ...
            sended.value=name.options[name.selectedIndex].value+"-"+adress.value+"-"+detail.value;
        })
...
function confirm_form(){
    ...
    if(document.querySelector(".confirm_form")){
        ...
        let ok_confirm=false, ok_file=false;
        ...
        formtype=="sell_crypto" && addEvent(custom_sell_method,'change',()=>{


            document.querySelector("#sell_pay_address").textContent=custom_sell_method.options[custom_sell_method.selectedIndex].value;
        ...
        formtype=="buy_crypto" && addEvent(custom_buy_method,'change',()=>{

            document.querySelector("#buy_pay_address").value=custom_buy_method.options[custom_buy_method.selectedIndex].value;
        ...
        function wallet_confirm(){
            ...
            if(confirmW.value!=document.querySelector('.wallet_address').value){
                ...
                document.querySelector("#wallet-error").textContent="Les adresses de paiement sont differentes";
        ...
        addEvent(selected_pay_method,'change',()=>{
            ...
            pay_method.textContent=selected_pay_method.options[selected_pay_method.selectedIndex].textContent;

            document.querySelectorAll(".data-deposit").forEach((element)=>{

                if(element.getAttribute("data-deposit_name")==pay_method.textContent){
            ...
            formtype=="sell_crypto" && custom_paiement_manager(selected_pay_method.options[selected_pay_method.selectedIndex].textContent)
        })

        addEvent(selected_pay_amount,'input',()=>{pay_amount.textContent=total_amount.value});
        ...
        addEvent(coin,'change',()=>{
            ...
            document.querySelector(".modal-body img").src="/assets/qr_code/"+coin.options[coin.selectedIndex].getAttribute("data-qr");
        ...
        addEvent(receipt,"change",()=>{
            ...
            {
                document.querySelector("#receipt-error").textContent="Le fichier que vous avez selectionné n'est pas pris en charge, veillez choisis une image au format jpg, gif, png, webp, ou jpeg.";
        ...
        confirmW && addEvent(confirmW,'change',()=>{
        ...
        addEvent(form,'submit',(event)=>{
            ...
            (ok_confirm && ok_file) ? ()=> { document.createElement('form').submit.call(form) } :event.preventDefault();
        ...
        document.querySelectorAll('.crypto-form').forEach((form)=>{
            ...
            addEvent(coin,'change',()=>{
                ...
                if(form.getAttribute("id")=="sell_crypto"){
                    ...
                    adress.value=coin.options[coin.selectedIndex].getAttribute('data-wallet_address');

                    let label=adress.parentNode.querySelector("label");

                    label.innerHTML=coin.options[coin.selectedIndex].textContent+" wallet address";
        ...
        if(check.length>0){
            ...
            for (const key in check) {
                ...
                if (Object.hasOwnProperty.call(check, key)) {
                    ...
                    option.setAttribute("value",check[key].name+":"+check[key].number);
            ...
            addEvent(closes,"click",()=>{
                ...
                document.querySelector("#buy_pay_address").setAttribute("name","wallet_address")
...
function transaction(){
    ...
    if(btns){
        ...
        let sell_only=modal.querySelector(".sell_only"),buy_only=modal.querySelector(".buy_only");
            ...
            addEvent(btn,'click',()=>{
                ...
                menu_item.forEach(item=>{
                    ...
                    addEvent(item,"click",()=>{
                        ...
                        type=="menu_send" && (console.log("ok"), detail_container.style.display="none",admin_image_container.style.display="none",user_image_container.style.display="block");
                        type=="menu_get" && (detail_container.style.display="none",user_image_container.style.display="none",admin_image_container.style.display="block");
                        type=="menu_detail" && (user_image_container.style.display="none",admin_image_container.style.display="none",detail_container.style.display="block");
